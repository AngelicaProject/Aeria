...
- **Initialize** runs `git init` (branch `main` unless `init.defaultBranch`
  ...
  `.gitattributes`, so Workspace Format v1 files stay LF-only even with
...
Because Workspace Format v1 stores one unit per JSONL line in a shard chosen
...
- **Unit history** lists the commits that changed one unit's record, newest
  ...
  that is not valid Workspace Format v1 data is reported as invalid, never
...
All shard and record decoding goes through the Workspace Format v1 reader in
`aeria-workspace`; Git history never bypasses format validation.
...
workspace to a different source snapshot; source updates remain the explicit
rebase workflow.
...
Format v1 reader, and merges them per `TranslationUnitId`. Edits to
...
3. A source binding or fingerprint change on either side conflicts with any
   other change on the other side; source updates are rebase work.
