...
impl SourcePackage {
    ...
    /// Moves the runtime package path and its disposable verification state
    /// after the package has already been fully validated.
    ///
    /// The process-local and persistent verification records are moved to the
    /// final package path. This does not reopen the archive or repeat any HSP,
    /// HXS, or HSG validation. Failure to write the disposable record does not
    /// invalidate the already-published package; the next reopen falls back
    /// to complete validation.
    #[must_use]
    pub fn relocate_package_path(
        mut self,
        package_path: impl Into<PathBuf>,
        cache_root: impl AsRef<Path>,
    ) -> Self {
        let package_path = package_path.into();
        crate::reader::relocate_verified_cache(
            &self.package_path,
            &package_path,
            cache_root.as_ref(),
        );
